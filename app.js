...
function formatAgeRange(camp) {
  const hasMin = Number.isInteger(camp.ageMin);
  const hasMax = Number.isInteger(camp.ageMax);
  if (hasMin && hasMax) {
    return `${camp.ageMin}-${camp.ageMax}`;
  }
  if (hasMin) {
    return `${camp.ageMin}+`;
  }
  if (hasMax) {
    return `Up to ${camp.ageMax}`;
  }
  return "Check source";
}
...
function renderCamps(camps) {
  ...
  camps.forEach((camp) => {
    ...
    fragment.querySelector(".camp-name").textContent = camp.name;
    fragment.querySelector(".camp-summary").textContent = camp.type;
    fragment.querySelector(".camp-county").textContent = camp.county;
